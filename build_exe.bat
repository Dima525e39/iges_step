...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$date=(Get-Date).ToString('yyyy-MM-dd HH:mm:ss'); Set-Content -Encoding UTF8 version.txt @('TubeCutCalculator v0.4.7','Build date: ' + $date,'Description: STEP/IGES import, 3D viewer, B-Rep cut length from CUT_FEATURE/CUT_END only, cut-end/feature split, debug_edges.csv, pierces, wall thickness confidence, manual thickness, and price estimate.')"
if errorlevel 1 exit /b 1
