...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$date=(Get-Date).ToString('yyyy-MM-dd HH:mm:ss'); Set-Content -Encoding UTF8 version.txt @('TubeCutCalculator v0.3.0','Build date: ' + $date,'Description: STEP/IGES import, 3D viewer, drag-and-drop, file queue, vertical viewer resize, basic geometry analysis, and temporary DEV script window.')"
if errorlevel 1 exit /b 1
