...
def install_top_menu(window: QMainWindow) -> None:
    ...
    _add_action(settings_menu, "Шаблон коммерческого предложения", window._open_offer_settings)
    ...
    _add_action(export_menu, "Печать коммерческого предложения", window._print_commercial_offer)
