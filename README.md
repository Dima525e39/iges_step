...
Текущая версия: **v0.2.3**.

## Что работает в v0.2.3
...
Расчет длины реза, количества врезок, толщины стенки, цены и 2D-развертка в v0.2.3 намеренно не реализованы. Эти поля остаются заглушками до этапа анализа геометрии.
...
Для v0.2.3 рекомендуется Miniforge или Anaconda, потому что `pythonocc-core` устанавливается из `conda-forge`.
...
Workflow `.github/workflows/build-windows.yml` собирает EXE на Windows runner при push в `main`, push в `feature/**` и при ручном запуске. После сборки workflow запускает smoke-test импортов внутри готового EXE. При создании тега вида `v0.2.3` EXE также прикладывается к GitHub Release.
