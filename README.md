...
Расчет длины реза, количества врезок, толщины стенки, цены и 2D-развертка в v0.2.0 намеренно не реализованы. Эти поля остаются заглушками до этапа анализа геометрии.
...
Для v0.2.0 рекомендуется Miniforge или Anaconda, потому что `pythonocc-core` устанавливается из `conda-forge`.
...
Workflow `.github/workflows/build-windows.yml` собирает EXE на Windows runner при push в `main`, push в `feature/**` и при ручном запуске. При создании тега вида `v0.2.0` EXE также прикладывается к GitHub Release.
