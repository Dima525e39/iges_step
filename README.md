...
Расчет толщины стенки и 2D-развертка в v0.4.0 намеренно не реализованы. Длина реза и врезки считаются предварительно по эвристике наружных ребер; результат нужно проверять через DEV-окно на реальных трубах.
...
Для v0.4.0 рекомендуется Miniforge или Anaconda, потому что `pythonocc-core` устанавливается из `conda-forge`.
...
Workflow `.github/workflows/build-windows.yml` собирает EXE на Windows runner при push в `main`, push в `feature/**` и при ручном запуске. После сборки workflow запускает smoke-test импортов внутри готового EXE. При создании тега вида `v0.4.0` EXE также прикладывается к GitHub Release.
