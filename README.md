...
Текущая версия: **v0.4.4**.

## Что работает в v0.4.4
...
- Предварительный расчет длины реза по наружным ребрам граней толщины изделия.
...
2D-развертка в v0.4.4 намеренно не реализована. Толщина, длина реза и врезки считаются предварительно по наружным контурам граней толщины; результат нужно проверять через DEV-окно на реальных трубах.
...
Для v0.4.4 рекомендуется Miniforge или Anaconda, потому что `pythonocc-core` устанавливается из `conda-forge`.
...
Workflow `.github/workflows/build-windows.yml` собирает EXE на Windows runner при push в `main`, push в `feature/**` и при ручном запуске. После сборки workflow запускает smoke-test импортов внутри готового EXE. При создании тега вида `v0.4.4` EXE также прикладывается к GitHub Release.
