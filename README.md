...
Текущая версия: **v0.4.2**.

## Что работает в v0.4.2
...
- Предварительный расчет длины реза по кандидатам ребер на наружной поверхности трубы.
- Предварительный расчет количества врезок как количества найденных контуров реза.
- Учет вырезов, которые переходят через несколько наружных граней профильной трубы.
- Отбор реза по границам наружная поверхность/грань выреза и wire-петлям граней.
- Предварительная стоимость по текущим настройкам цены за метр и врезку.
...
Расчет толщины стенки и 2D-развертка в v0.4.2 намеренно не реализованы. Длина реза и врезки считаются предварительно по эвристике наружных ребер и wire-контуров; результат нужно проверять через DEV-окно на реальных трубах.
...
Для v0.4.2 рекомендуется Miniforge или Anaconda, потому что `pythonocc-core` устанавливается из `conda-forge`.
...
Workflow `.github/workflows/build-windows.yml` собирает EXE на Windows runner при push в `main`, push в `feature/**` и при ручном запуске. После сборки workflow запускает smoke-test импортов внутри готового EXE. При создании тега вида `v0.4.2` EXE также прикладывается к GitHub Release.
