...
Текущая версия: **v0.4.3**.

## Что работает в v0.4.3
...
- Предварительный расчет длины реза по площади граней реза в толщине изделия.
- Предварительный расчет количества врезок как количества связанных групп толщинных граней.
- Учет вырезов, которые переходят через несколько наружных граней профильной трубы.
- Первичная проверка профильных труб со скругленными наружными гранями через наружные продольные поверхности.
...
2D-развертка в v0.4.3 намеренно не реализована. Толщина, длина реза и врезки считаются предварительно по эвристике толщинных граней; результат нужно проверять через DEV-окно на реальных трубах.
...
Для v0.4.3 рекомендуется Miniforge или Anaconda, потому что `pythonocc-core` устанавливается из `conda-forge`.
...
Workflow `.github/workflows/build-windows.yml` собирает EXE на Windows runner при push в `main`, push в `feature/**` и при ручном запуске. После сборки workflow запускает smoke-test импортов внутри готового EXE. При создании тега вида `v0.4.3` EXE также прикладывается к GitHub Release.
